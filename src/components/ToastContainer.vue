...
<template>
  <div class="fixed top-24 right-4 z-[9999] space-y-2 pointer-events-none max-w-[calc(100vw-2rem)]">
    <TransitionGroup name="toast">
      ...
      >
        <component :is="getIcon(toast.type)" :size="20" class="flex-shrink-0 text-black" />
        <p class="flex-1 text-black font-medium">{{ toast.message }}</p>
        <button
          @click="removeToast(toast.id)"
          class="flex-shrink-0 text-black/80 hover:text-black transition"
        >
          <X :size="18" />
        </button>
      </div>
