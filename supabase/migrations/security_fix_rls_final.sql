...
DROP POLICY IF EXISTS "users_admin_role_update" ON users;
...
-- Policy: Users can only update their own profile (non-sensitive fields)
...
DROP POLICY IF EXISTS "bounties_select_all" ON bounties;
DROP POLICY IF EXISTS "bounties_insert_authenticated" ON bounties;
DROP POLICY IF EXISTS "bounties_update_own" ON bounties;
DROP POLICY IF EXISTS "bounties_admin_update" ON bounties;
...
-- Policy: Users can update their own bounties (if active)
...
DROP POLICY IF EXISTS "claims_select_own_or_approved" ON bounty_claims;
DROP POLICY IF EXISTS "claims_select_approved_anon" ON bounty_claims;
DROP POLICY IF EXISTS "claims_insert_own" ON bounty_claims;
DROP POLICY IF EXISTS "claims_admin_update" ON bounty_claims;
DROP POLICY IF EXISTS "claims_delete_own_pending" ON bounty_claims;
...
-- ============================================================================
-- BOUNTY_HUNTERS TABLE (not "hunters")
-- ============================================================================

DROP POLICY IF EXISTS "Anyone can view hunters" ON bounty_hunters;
DROP POLICY IF EXISTS "Anyone can join hunts" ON bounty_hunters;
DROP POLICY IF EXISTS "Anyone can leave hunts" ON bounty_hunters;
DROP POLICY IF EXISTS "hunters_select_all" ON bounty_hunters;
DROP POLICY IF EXISTS "hunters_insert_own" ON bounty_hunters;
DROP POLICY IF EXISTS "hunters_delete_own" ON bounty_hunters;

ALTER TABLE bounty_hunters ENABLE ROW LEVEL SECURITY;

-- Policy: Anyone can view who's hunting (for counter displays)
CREATE POLICY "bounty_hunters_select_all"
  ON bounty_hunters FOR SELECT
  TO authenticated, anon
  USING (true);

-- Policy: Users can join hunts
CREATE POLICY "bounty_hunters_insert_own"
  ON bounty_hunters FOR INSERT
  TO authenticated
  WITH CHECK (auth.uid() = hunter_id);

-- Policy: Users can leave their own hunts
CREATE POLICY "bounty_hunters_delete_own"
  ON bounty_hunters FOR DELETE
  TO authenticated
  USING (auth.uid() = hunter_id);

-- ============================================================================
-- KILLS TABLE (Proud Rat kills)
-- ============================================================================

DROP POLICY IF EXISTS "kills_select_all" ON kills;
DROP POLICY IF EXISTS "kills_insert_proud_rat" ON kills;
DROP POLICY IF EXISTS "kills_update_own" ON kills;
DROP POLICY IF EXISTS "kills_admin_verify" ON kills;

ALTER TABLE kills ENABLE ROW LEVEL SECURITY;

-- Policy: Anyone can view approved kills (for leaderboard)
CREATE POLICY "kills_select_all"
  ON kills FOR SELECT
  TO authenticated, anon
  USING (true);

-- Policy: Only Proud Rats can submit kills
CREATE POLICY "kills_insert_proud_rat"
  ON kills FOR INSERT
  TO authenticated
  WITH CHECK (
    auth.uid() = killer_id
...
CREATE POLICY "kills_update_own"
  ON kills FOR UPDATE
  TO authenticated
  USING (killer_id = auth.uid() AND verification_status = 'pending')
  WITH CHECK (killer_id = auth.uid());

-- Policy: Admins can verify kills
CREATE POLICY "kills_admin_verify"
  ON kills FOR UPDATE
  TO authenticated
  USING (
    EXISTS (
...
DROP POLICY IF EXISTS "rat_of_day_select_all" ON rat_of_the_day;
DROP POLICY IF EXISTS "rat_of_day_admin_insert" ON rat_of_the_day;
DROP POLICY IF EXISTS "rat_of_day_admin_update" ON rat_of_the_day;
DROP POLICY IF EXISTS "rat_of_day_admin_delete" ON rat_of_the_day;
...
DROP POLICY IF EXISTS "achievements_insert_system" ON user_achievements;
...
-- ============================================================================
-- ROLE_POLL_VOTES TABLE
-- ============================================================================

DROP POLICY IF EXISTS "role_poll_votes_select_own" ON role_poll_votes;
DROP POLICY IF EXISTS "role_poll_votes_insert_own" ON role_poll_votes;
DROP POLICY IF EXISTS "role_poll_votes_update_own" ON role_poll_votes;

ALTER TABLE role_poll_votes ENABLE ROW LEVEL SECURITY;

-- Policy: Users can view their own vote
CREATE POLICY "role_poll_votes_select_own"
  ON role_poll_votes FOR SELECT
  TO authenticated
  USING (user_id = auth.uid());

-- Policy: Users can insert their own vote
CREATE POLICY "role_poll_votes_insert_own"
  ON role_poll_votes FOR INSERT
  TO authenticated
  WITH CHECK (user_id = auth.uid());

-- Policy: Users can update their own vote
CREATE POLICY "role_poll_votes_update_own"
  ON role_poll_votes FOR UPDATE
  TO authenticated
  USING (user_id = auth.uid())
  WITH CHECK (user_id = auth.uid());
...
BEGIN
  FOR r IN
    ...
    AND tablename IN (
      'users', 'bounties', 'bounty_claims', 'bounty_hunters',
      'rat_of_the_day', 'bug_reports', 'admin_logs',
      'achievements', 'user_achievements', 'kills', 'role_poll_votes'
    )
  LOOP
